...
This is a controlled Claude command definition for assisting the documentary reading of approved specs. It is a definition only. It contains no frontmatter, no YAML, no JSON, no machine-readable contract, and no pseudo-schema. It performs no execution and authorizes no execution.
...
This command exists only to guide, at a documentary and assistive level, the reading of approved specs. It orients a human or reviewer toward a structured, documentary reading of what an approved spec authorizes, in alignment with the current readiness statement, the authorized source documents, the negative scope, the future task gate, and the non-execution boundaries. It produces no result by itself, runs nothing, and changes nothing.
...
This command only guides the assistive reading of approved specs and does not automatically execute the subagent. It does not call, trigger, schedule, or run `.claude/agents/spec-reader-subagent.md`; it only points to it as the related controlled subagent definition.

## Related Institutional Skill

`read-approved-specs`

This command is related only to the institutional skill `read-approved-specs`. It is not related to any other skill in this task.

## Allowed Use

This command may only be used conceptually to guide a documentary reading of:
...
* the paths of the authorized source documents;
...
* declaring success without documentary evidence.
...
3. confirm the authorized source documents;
4. review the indicated approved specs;
5. identify the readiness statements found;
6. identify the negative scope;
7. identify the future task gate;
8. record evidence gaps;
9. recommend stopping if the evidence is insufficient;
...
This command is aware that the current readiness statement governs what is authorized. If the readiness statement found does not match the expected readiness statement, or is absent, the command guides the reviewer to recommend stopping and to report the discrepancy. The command never guides a finalized reading conclusion when the readiness statement is incompatible.
