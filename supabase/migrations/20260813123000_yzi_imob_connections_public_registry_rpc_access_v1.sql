-- YZI IMOB - Connections public registry RPC access v1
--
-- Keep the public registry RPC as SECURITY INVOKER and avoid granting
-- authenticated users SELECT on tenant_connections.vault_secret_id. The public
-- projection only needs a tenant-safe boolean that a Vault reference exists.

begin;

create schema if not exists yzi_imob_connections_private authorization postgres;

revoke all on schema yzi_imob_connections_private
from public, anon, authenticated;

grant usage on schema yzi_imob_connections_private to authenticated;

create or replace function yzi_imob_connections_private.has_connection_vault_secret(
  p_tenant_id uuid,
...
revoke all on function yzi_imob_connections_private.has_connection_vault_secret(uuid, uuid)
from public, anon, authenticated;

grant execute on function yzi_imob_connections_private.has_connection_vault_secret(uuid, uuid)
to authenticated;
...
stable
security invoker
set search_path = pg_catalog, public, auth, yzi_imob_connections_private
as $$
declare
...
begin
  ...
  with connection_rows as (
    select
      ...
      tc.updated_at,
      case
        when tc.provider = 'metricool' then yzi_imob_connections_private.has_connection_vault_secret(tc.tenant_id, tc.id)
        else false
      end as public_has_vault_secret,
      case
  ...
  normalized as (
    select
      ...
      (
        ...
        and cr.public_has_vault_secret
...
comment on function yzi_imob_connections_private.has_connection_vault_secret(uuid, uuid) is
  'Tenant-safe private boolean helper for the public Connections Registry. Returns only whether an eligible connection has a Vault secret reference; never returns the Vault identifier or secret value.';

comment on function public.get_yzi_imob_tenant_connections(uuid) is
  'Admin-only read projection for YZI IMOB Connections. Requires explicit p_tenant_id and active owner/admin membership in an active tenant. Runs as SECURITY INVOKER, uses RLS for public table reads, and never returns vault_secret_id or internal secret values.';
