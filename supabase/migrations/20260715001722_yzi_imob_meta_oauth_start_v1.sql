-- YZI IMOB - Meta OAuth Start v1
-- Supabase migration: 20260715001722_yzi_imob_meta_oauth_start_v1
--
-- Scope:
-- - Governed RPC to create one Meta OAuth authorization attempt.
-- - Stores only SHA-256 state_hash.
-- - Writes sanitized authorization_started audit metadata.
-- - catalog_id is the requested catalog entry point for one Meta provider
--   authorization, not a separate tenant_connection identity.
--
-- Non-goals:
-- - No callback handling.
-- - No token exchange.
-- - No Vault writes.
-- - No asset discovery or tenant_connection creation.
...
begin
  ...
  if p_catalog_id is null
    or p_catalog_id <> any (array['instagram', 'facebook', 'meta-ads']::text[])
  then
